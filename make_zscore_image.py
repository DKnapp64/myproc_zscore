...
def main(tile, ascdesc, stopat):
  ...
  ## build week directory name
  startdate = (datetime.date(int(stopat[0:4]), int(stopat[4:6]), \
    int(stopat[6:8])) + datetime.timedelta(days=-7)).strftime('%Y%m%d')
  weekdir = startdate+'_to_'+stopat
  ...
  infile = weekdir + '/' + ascdesc + '/' + tile + '_br_comp.tif'
  outfile = weekdir + '/' + ascdesc + '/' + tile + '_' + stopat + '_zscore_base.tif'
  ## statfile = 'BaseFiles/' + ascdesc + '_' + tile + '_base.tif'
  statfile = 'BaseFiles/' + tile + '_base.tif'

  ## coralfile = 'CoralNew/' + tile + '_coral3.tif'
  coralfile = 'CoralNew/' + tile + '_coral.tif'
...
if __name__ == "__main__":
  ...
  if len( sys.argv ) != 4:
    print("[ ERROR ] you must supply 4 arguments: make_zscore_image2.py tile ascdesc stopat")
    print("where:")
